...
/*
*Field to know the type of lattice cell.
*/
...
/*
 * This enum represents result of comparing input-query with eqclass' bounds
 * CONTAIN_UP means the query may be new upper iff its result is same as eq-
 *            -class' result. 
 * CONTAINED_BY_UP means the query may be a child eqclass iff its result is 
 *                 contained by eqclass.
 * CONTAIN_BY_DOWN means the query may be a parent eqclass iff its result 
 *                 containes eqclass.
 * CONTAIN_DOWN means the query may be new down iff its result is same as e-
 *              -qclass.
 */
typedef enum compare_result{
	CMPDEFAULT,
	CONTAIN_UP,        //up implies query 
	PART_CONTAINUP,
	CONTAINED_BY_UP,   //query implies up 
	CONTAIN_DOWN,      //down implies query
	CONTAINED_BY_DOWN, //query implies down
	PART_CONTAINBYDOWN,
	NEWUPPER,          //new upper under the case that cmp_res is CONTAIN_UP before
	PARAUPPER,         //merge upper under the case that cmp_res is CONNTAIN_DOWN
	NEWDOWN,
	PARADOWN,
	IRRELEVANT,
	ANSWERED
