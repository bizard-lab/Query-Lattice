...
#include "postgres.h"
#include "/Users/flamecat/z3-master/src/api/z3.h"
...
/*--------
 *Nodes to support functions of lattice
 *--------
 */
typedef struct pos_lattice
{
	Datum c_uclass;
    Datum c_dclass;
	Datum s_class;
...
/*
 * basic nodes of lattice
 */
typedef struct LCell_u
{
	Z3_ast      sr;
	struct EqualClass     *parent; //To indicate partial order among classes.
	struct EqualClass     *child; //same as above
//can add metadata of cell_l
}LCell_u;

typedef struct LCell_l
{
	Z3_ast      sr;
	struct EqualClass     *parent; //To indicate partial order among classes.
	struct EqualClass     *child; //same as above
...
/*--------
*My primary node definitions of everything about lattice.
*---------
*/
...
typedef struct EqualClass
{
	CellTag         type;
	classid         cid; //this id is specified when space is allocated 
	int             ulength;// indicate number of upper cells
	int             llength;
	int             accessed; //Accessed by sql queries.
	int             tts_max_length;
	char           *col_key; //Key to build hash table.
	Z3_context      z3_ctx;
	LCell_u         ucell[4];
	LCell_l         lcell[16];
	/* 
	 * code for fetching result table of the class(in form of tts), may be replaced by
	 * my-defined type to store necessary info of tuple and save memory
	 */
    Datum  tts_desc;
	List *slot;
//	struct HTAB     *c_hashtbl;//compare by hashing when tuples are too many(>30)
	
	struct EqualClass     *bro; //to indicate classes with same key but irrelevant sr
}EqualClass;

/*
 *Extended List as my Lattice 
 */
